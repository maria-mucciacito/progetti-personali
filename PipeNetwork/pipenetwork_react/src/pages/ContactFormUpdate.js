...
export const ContactFormUpdate = () => {
  ...
  useEffect(() => {
    axios
      ...
      .then((result) => {
        ...
        setBirthdayDate(result?.data.birthdayDate);
  ...
  const updateContact = async (e) => {
    ...
    try {
      await axios
        .put(UPDATE_CONTACT + contactId, json, {
          headers: { "Content-Type": "application/json" },
        })
        .then((result) => {
          if (result.status === 204) {
            setMsg("Il contatto è stato aggiornato con successo!");
          }
        })
        .catch((error) => console.log(error));
      setLastName("");
      ...
      <CDBCard style={{ width: "50rem" }}>
        <CDBCardBody className="mx-4">
          <div className="text-center mt-4 mb-4 m-4">
            <p className="h4"> Update Contact </p>
          </div>
